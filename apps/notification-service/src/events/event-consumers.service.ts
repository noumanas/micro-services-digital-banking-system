...
const TOPICS = ["AccountCreated", "KycApproved", "KycRejected", "TransferCompleted", "TransferFailed"].map(
  topicForEvent,
);
...
export class EventConsumersService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit(): Promise<void> {
    ...
    await this.consumer.run((event) => this.notificationsService.handleEvent(event), {
      topics: TOPICS,
      maxRetries: 3,
