...
const CONSUMER_NAME = "transfer-service";
const TOPICS = ["AccountCreated", "TransferApproved", "TransferFlagged", "TransferBlocked"].map(topicForEvent);
      ...
      async (event) => {
        switch (event.eventType) {
          case "AccountCreated": {
            ...
            await this.transfersService.handleAccountCreated({
          ...
          case "TransferFlagged":
          case "TransferBlocked":
            await this.transfersService.handleFraudDecision(event as never);
            return;
          default:
