...
const CONSUMER_NAME = "customer-service";
const TOPICS = ["UserRegistered", "KycApproved", "KycRejected"].map(topicForEvent);
    ...
    await this.consumer.run(
      async (event) => {
        switch (event.eventType) {
          case "UserRegistered":
            await this.customersService.handleUserRegistered(event as never);
            return;
          case "KycApproved":
            await this.customersService.handleKycDecision(event as never, true);
            return;
          case "KycRejected":
            await this.customersService.handleKycDecision(event as never, false);
            return;
          default:
